...
const dbName = 'put-your-database-name-here';
...
app.get('/strategic-risk-management-data', async (req, res) => {
    try {
        await initializeDocument('strategic-risk-management', { reviews: [] });
        const data = await db.get('strategic-risk-management');
        
        // Ensure the 'reviews' field is returned even if it's empty
        res.status(200).json({ reviews: data.reviews || [] });
    } catch (err) {
        console.error('Error retrieving strategic risk management data:', err);
        res.status(500).json({ error: 'Failed to retrieve data' });
    }
});
...
// Save Defined Cybersecurity Roles Route
...
app.get('/defined-cybersecurity-roles-data', async (req, res) => {
    try {
        await initializeDocument('defined-cybersecurity-roles', { 
            enterpriseRiskManagement: '', 
            peopleList: [], 
            seniorManagementInvolvement: '', 
            escalationThresholds: '', 
            informInvolved: '', 
            escalationTimeline: '', 
            responsibleEscalation: '', 
...
// Get database name for the feature for downloading the database
app.get('/get-db-name', (req, res) => {
    res.json({ dbName });  // Sends the database name to the client
});

// Download database feature

app.get('/download-db/:dbName', async (req, res) => {
    try {
        const dbName = req.params.dbName; // Get dbName from route parameter

        const db = couch.use(dbName);  // Use the dynamic dbName

        // Fetch all documents from the database
        const allDocs = await db.list({ include_docs: true });
        const documents = allDocs.rows.map(row => row.doc);  // Extract the docs

        // Convert to JSON
        const jsonContent = JSON.stringify(documents, null, 2);
        const fileName = `couchdb-backup-${dbName}-${new Date().toISOString()}.json`;
        ...
        console.error('Error fetching database for download:', err);
        res.status(500).json({ error: 'Failed to download database' });
    }
});

// Onboarding Procedure Routes

// Save onboarding question for a vendor
app.post('/save-onboarding', async (req, res) => {
    try {
        const { vendor, onboardingQuestionData } = req.body;

        // Fetch the existing onboarding document or create a new one
        let onboardingData = await db.get('onboarding-procedure').catch(() => ({ onboarding: {} }));

        // Save the onboarding question for the specified vendor
        onboardingData.onboarding[vendor] = onboardingQuestionData;

        const response = await db.insert({
            _id: 'onboarding-procedure',
            _rev: onboardingData._rev,
...
app.get('/onboarding-data', async (req, res) => {
    try {
        await initializeDocument('onboarding-procedure', { onboarding: {} });
        const data = await db.get('onboarding-procedure');
        res.status(200).json(data);
    } catch (err) {
...
// Offboarding Procedure Routes

// Save offboarding question for a vendor
app.post('/save-offboarding', async (req, res) => {
    try {
        const { vendor, offboardingQuestionData } = req.body;

        // Fetch the existing offboarding document or create a new one
        let offboardingData;
        try {
            // Try to fetch the document
            offboardingData = await db.get('offboarding-procedure');
        } catch (err) {
            // If document doesn't exist, initialize a new document
            offboardingData = { _id: 'offboarding-procedure', offboarding: {} };
        }

        // Save the offboarding question for the specified vendor
        offboardingData.offboarding[vendor] = offboardingQuestionData;

        // Insert the updated document with or without _rev (depending on whether it was fetched or newly created)
        const response = await db.insert({
            _id: 'offboarding-procedure',
            _rev: offboardingData._rev || undefined, // Only include _rev if it exists
            offboarding: offboardingData.offboarding,
        });

        res.status(200).json(response);
    } catch (err) {
        console.error('Error saving offboarding data:', err);
        res.status(500).json({ error: 'Failed to save offboarding data' });
    }
});

// Route to retrieve offboarding data
app.get('/offboarding-data', async (req, res) => {
    try {
        // Initialize document if it doesn't exist
        await initializeDocument('offboarding-procedure', { offboarding: {} });

        // Fetch the offboarding data
        const data = await db.get('offboarding-procedure');
        res.status(200).json(data);
    } catch (err) {
        console.error('Error retrieving offboarding data:', err);
        res.status(500).json({ error: 'Failed to retrieve offboarding data' });
    }
});
