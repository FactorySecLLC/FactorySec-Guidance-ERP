...
document.addEventListener('DOMContentLoaded', function() {
    // Fetch and display the saved cybersecurity roles data for GV.RM-03
    fetch('/defined-cybersecurity-roles-data')  // Ensure this endpoint is correct for your data source
        .then(response => response.json())
        .then(data => {
            const rolesOutput = document.getElementById('saved-cybersecurity-roles-output');
            if (rolesOutput) {
                if (data.roles && data.roles.length > 0) {
                    const rolesHtml = data.roles.map(role => `
                        <p>
                            <strong>Name:</strong> ${role.name}<br>
                            <strong>Responsibilities:</strong> ${role.responsibilities}<br>
                            <strong>Authority:</strong> ${role.authority}
                        </p>
                    `).join('');
                    rolesOutput.innerHTML = rolesHtml || 'No roles data found.';
                } else {
                    rolesOutput.textContent = 'No saved cybersecurity roles data available.';
                }
            }
        })
        .catch(error => {
            console.error('Error fetching the saved cybersecurity roles:', error);
            const rolesOutput = document.getElementById('saved-cybersecurity-roles-output');
            rolesOutput.textContent = 'Failed to load saved cybersecurity roles data.';
        });
});
